...
function generate(from, to, step) {
  if ((to - from) * step < 0) return []
  return [from].concat(generate(from += step, to, step));
}

function ArrayComprehension(options) {
  generator = options.generator;
  if (!generator || !generator.trim()) return [];
  if (!/\./.test(options.generator)) return generator.split(',').map(Number);
  
  generator = generator.match(/^((\d+),(\d+)|(\d+))\.\.(\d+)$/);
  if (generator) {
    generator = generator.map(Number);
    var step = isNaN(generator[2]) ? 1 : generator[3] - generator[2];
    return generate(generator[4] || generator[2], generator[5], step);
  }
  return null;
}
