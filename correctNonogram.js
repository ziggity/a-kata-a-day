...
*/
function verify(v, ofs) {
    const numbers = v.slice(0, ofs).filter(v => v !== "-");
    const squares = v.slice(ofs);
    var nidx = 0;
    var rlen = 0;
    ...
    for (var i = 0; i < squares.length; i++) {
        if (squares[i] === '#') {
            rlen++;
        } else if (rlen && numbers[nidx++] != rlen) { 
            return false;
        } else {
            rlen = 0;
        }
    }
    if (rlen && numbers[nidx++] != rlen) { 
        return false;
    }
    return nidx === numbers.length;
}
...
function correctNonogram(size, nonogramField) {
    ...
    while(nonogramField[nonogramField.length - 1][ofs].match(/\d|-/)) {
        ofs++;
    }
    
    for (var i = ofs; i < nonogramField.length; i++) {
        if (!verify(nonogramField[i], ofs)) {
            return false;
        }
    }
    for (var i = ofs; i < nonogramField.length; i++) {
        if (!verify(nonogramField.map(v => v[i]), ofs)) {
            return false;
        }
    }
